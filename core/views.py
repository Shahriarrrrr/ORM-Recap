...
from .models import Resturant, Sale, Rating, StaffResturant
...
"""
Enhancements for many to many

"""
def index(request):
    #jobs = StaffResturant.objects.all() THIS would create duplicate queries
    jobs = StaffResturant.objects.prefetch_related('resturant', 'staff')

    for job in jobs:
        print(job.resturant.name)
